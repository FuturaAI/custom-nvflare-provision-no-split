...
from pt.utils.custom_data import CustomDataset, IMAGES_ROOT
...
class CustomLearner(Learner):  
    def __init__(
        ...
        Args:
            train_idx_root: directory with site training indices data.
    ...
    def _create_datasets(self, fl_ctx: FLContext):
        if self.train_dataset is None or self.train_loader is None:
            # If not central (default case), use direct data loading
            self.train_dataset = CustomDataset(
                data_folder=f'{IMAGES_ROOT}/train',
